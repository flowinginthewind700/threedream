...
- 回退档的性能目标：验收写的是「性能目标明确降级」，所以降级要是一个数字而不是一句
  道歉。`scripts/bench_cpu_soft.ts` 用与 `bench_gpu_soft.mjs` 同一条阶梯（cloth、
  8 iterations、seed 4242、每档 160 步、20 个样本）跑 `softCpu.ts` 本身，本机 p50 读到
  1k / 5k / 10k / 20k 分别 1.5 / 7.9 / 15.9 / 32.1 ms/步，拟合出每节点约 1.61 us、
  固定开销约等于 0（-0.08 ms），p95 与 p50 相差在 1% 以内 —— 计时区间里没有驱动提交
  也没有队列 flush，这就是机器噪声底，因此 GPU 那张表里比它更宽的离散才是设备路径。
  对照设备档的每节点约 0.16 us，回退档每节点贵约十倍；1 万节点的 15.9 ms 已经是一整个
  60 Hz 帧，还没算绘制，所以回退档明确的目标是 60 Hz 下约 2k 节点（约 3.2 ms/步），
  而设备档的目标仍是 2 万。单线程是生产现实不是疏忽：GitHub Pages 不发 COOP/COEP，
  worker 没法与页面共享 `Float32Array`，而按节点切分的求解器要在每个 color 批次上同步，
  那正是并行度已经交给 GPU 的那些批次。
